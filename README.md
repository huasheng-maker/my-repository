...
- 通过 pull request 的方式提交到[课程仓库](https://github.com/huasheng-maker/my-repository)，具体位置为 `2025-GitPractice` 文件夹下以你 GitHub ID 命名的文件夹

## Pull Request

**本节通过实操考察 pull request 的掌握情况**

PR 考察分两阶段

1. 第一阶段要求在7月7日21:00前开启你的 PR
...
**提交：** 一个 MarkDown 文件和若干个图片文件（实际操作截图），在 MarkDown 中使用有效的`相对路径`引用你的图片


1. 请你在你原有的分支上，创建一个develop分支，使用`echo "just a test!" >> test.txt`,并切换到你原来的分支，然后合并俩个分支。
2. 请你将一个test.txt文件从pycharm上传至你自己的github仓库，在这里你可以了解到GitHub提交的基本流程。
3. 若你已经修改了部分文件、并且将其中的一部分加入了暂存区，应该如何回退这些修改，恢复到修改前最后一次提交的状态？给出至少两种不同的方式
